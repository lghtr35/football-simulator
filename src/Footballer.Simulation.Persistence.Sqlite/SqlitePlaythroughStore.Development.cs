...
t.Minutes-d.MinutesCheckpoint FROM DevelopmentData d JOIN Footballers p ON p.Id=d.FootballerId
...
WHERE ($after IS NULL OR p.Id>$after) AND d.LastDevelopmentDay<=$through ORDER BY p.Id LIMIT $limit;";
                ...
                {
                    var p = ReadFootballer(r); p.Overall = r.GetInt32(19); p.Skills.Goalkeeping = r.GetInt32(20);
...
StaminaRegen=$regen,Dribbling=$dribbling,FirstTouchControl=$touch,HitPower=$power,Accuracy=$accuracy,Tackling=$tackling,Strength=$strength WHERE Id=$id;",
                    ...
                    if (update.Change != 0) InsertHistory(c, tx, new WorldHistoryEntry { Day = day, Type = "development",
